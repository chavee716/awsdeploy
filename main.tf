...
resource "aws_security_group" "devops_sg" {
  name        = "My-security-group-new" # Changed name to avoid conflicts
  ...
  ingress {
    ...
    to_port     = 5173
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # Allow access to port 5173
  }
...
resource "aws_instance" "free_tier_instance" {
  ...
  user_data = <<-EOF
              ...
              newgrp docker                    # Re-attach to the docker group
              EOF
